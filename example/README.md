...
- [Using the `build` Folder](#using-the-build-folder)
  - [Changing the HTML](#changing-the-html)
  - [Adding Assets Outside of the Module System](#adding-assets-outside-of-the-module-system)
  - [When to Use the `build` Folder](#when-to-use-the-build-folder)
...
- `build/index.html` is the page template;
...
Only files inside `build` can be used from `build/index.html`.<br>
...
You can find the source HTML file in the `build` folder of the generated project. You may edit the `<title>` tag in it to change the title from “React App” to anything else.

Note that normally you wouldn’t edit files in the `build` folder very often. For example, [adding a stylesheet](#adding-a-stylesheet) is done without touching the HTML.
...
The `build` folder contains the HTML file so you can tweak it, for example, to [set the page title](#changing-the-page-title).
The `<script>` tag with the compiled code will be added to it automatically during the build process.

### Adding Assets Outside of the Module System

You can also add other assets to the `build` folder.
...
If you put a file into the `build` folder, it will **not** be processed by Webpack. Instead it will be copied into the build folder untouched. To reference assets in the `build` folder, you need to use a special variable called `PUBLIC_URL`.
...
Only files inside the `build` folder will be accessible by `%PUBLIC_URL%` prefix. If you need to use a file from `src` or `node_modules`, you’ll have to copy it there to explicitly specify your intention to make this file a part of the build.
...
- None of the files in `build` folder get post-processed or minified.
- Missing files will not be called at compilation time, and will cause 404 errors for your users.
- Result filenames won’t include content hashes so you’ll need to add query arguments or rename them every time they change.

### When to Use the `build` Folder

Normally we recommend importing [stylesheets](#adding-a-stylesheet), [images, and fonts](#adding-images-fonts-and-files) from JavaScript.
The `build` folder is useful as a workaround for a number of less common cases:
...
You can also access the environment variables starting with `REACT_APP_` in the `build/index.html`. For example:
...
[`build/manifest.json`](build/manifest.json), that you can customize with
details specific to your web application.

When a user adds a web app to their homescreen using Chrome or Firefox on
Android, the metadata in [`manifest.json`](build/manifest.json) determines what
...
If you’re using [Apache HTTP Server](https://httpd.apache.org/), you need to create a `.htaccess` file in the `build` folder that looks like this:
...
You can configure a custom domain with GitHub Pages by adding a `CNAME` file to the `build/` folder.
...
When you build the project, Create React App will place the `build` folder contents into the build output.
